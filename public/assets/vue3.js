var Vue = (function (exports) {
  ...
  function trigger(target, type, key, newValue, oldValue, oldTarget) {
    ...
    if (type === "clear" /* CLEAR */) {
      ...
      if (shouldTriggerIteration || (type === "set" /* SET */ && target instanceof Map)) {
  ...
  function createGetter(isReadonly = false, shallow = false) {
    return function get(target, key, receiver) {
      ...
      if (
        keyIsSymbol ? builtInSymbols.has(key) : key === `__proto__` || key === `__v_isRef`
      ) {
        return res;
      }
      if (!isReadonly) {
  ...
  function isSingleElementRoot(root, child) {
    const { children } = root;
    return (
      children.length === 1 && child.type === 1 /* ELEMENT */ && !isSlotOutlet(child)
    );
  }
